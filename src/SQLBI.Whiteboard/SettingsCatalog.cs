...
internal static class SettingsCatalog
{
    ...
    [
        new()
        {
            ...
            Description = "Design has every tool: the Insert tab and its palette, the shape, connector, and text tools, the bar above a selection, the lasso, and Duplicate. Teaching leaves them out, for a board you only annotate on; a shape, label, or connector already drawn is still drawn, still moved, still deleted, still exported either way. Custom is Teaching plus whichever of the four groups below are switched on.",
        ...
        new()
        {
            ...
            Description = "Say so at startup when Windows reports neither a pen tablet nor a touchscreen, and say what Mouse drawing gives you in place of a pen. A pen that has never been brought into range can be missing from what Windows reports, so turn this off if the notice appears on a machine that has one.",
        ...
        new()
        {
            ...
            Description = "Off keeps one finger for panning the board. On makes one finger draw with the current tool and puts Eraser and Pan on the toolbar; two fingers still pan and pinch-zoom. When no pen is detected chooses for you, drawing on a machine where Windows reports no pen and panning where it reports one.",
        ...
        new()
        {
            ...
            Description = "Off keeps the left button for moving and resizing containers. On makes it draw with the current tool and puts Eraser and Pan on the toolbar; Ctrl and the left button then move a container, and only Calligraphy varies its width, because a mouse reports no pressure. When there is no pen or touchscreen draws only on a machine where Windows reports neither; the pen is unaffected either way.",
        ...
        new()
        {
            ...
            Description = "Objects partly inside takes anything the area meets, which is how a quick sweep picks up a diagram. Only objects fully inside asks for the whole of a stroke or an object to be inside, which is what you want when the thing you are after sits among others. Drag with Select on empty canvas for a rectangle, or hold the Select button - or tap it again - for a freehand lasso; a frame is never taken either way.",
        ...
        new()
        {
            ...
            Description = "After the area has chosen, the selection can grow to the objects those ones touch. Ignore takes the area at its word; Single adds one round, so a label beside a picture comes along with it; Recursive keeps going until nothing more is added, which takes a whole connected diagram from one stroke in it. Strokes linked to a selected container come along whatever this is set to.",
        ...
        new()
        {
            ...
            Description = "Lines or Dots draw a faint grid under everything, spreading as you zoom in and stepping coarser as you zoom out, so how far in you are is visible on a board that is otherwise blank. It is drawn on screen only, and never appears in an export, a preview, or the Explorer thumbnail.",
        ...
        new()
        {
            ...
            Description = "On, a panel holds the eight shapes, the three connectors, and Text where you can reach them while you draw; the grip along its left edge drags it anywhere in the window with a mouse, a pen, or a finger, and it comes back where you left it. The pin at the end of the Insert row asks for the same thing as this row, and a right-click on the palette hides it again.",
        ...
        new()
        {
            ...
            Description = "Keep the tool leaves the shape, connector, or text tool active, so the next drag draws another of the same thing; Escape or any tool button still leaves it. Return to Select hands Select back as soon as one object has been drawn, with that object selected, so the next drag moves it rather than starting another.",
