...
Choose **Prompt** from the title-bar chip for AI instructions. Lines beginning with `- ` display as bullets; wrapped lines align with the item text, including space-indented lists. In F2 the original `- ` stays visible and editable, with the same hanging indentation. Copying and saving preserve the source text exactly. Prompt is never selected automatically on paste, and F6 does not rewrite it. This is list layout, not a full Markdown renderer.
